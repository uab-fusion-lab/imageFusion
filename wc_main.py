...
model = MNmodel.OneLayerCNN()
model_path = './model/naiveModel_NMIST.pt'
...
def fusion_train():
    ...
    model_clone_test = MNmodel.OneLayerCNN()
    ...
    num_epochs = 1
    for epoch in range(num_epochs):
        model_clone = MNmodel.OneLayerCNN()
        ...
        for i, (images, labels) in enumerate(label_data_loaders[1]):
            ...
            final_image = fmodel(avg_images.unsqueeze(0), model.conv1.weight.grad)
    ...
    def forward(self, input, target):  # do forward in the module.py
        ...
        for i in range(self.num_steps):
            ...
            with torch.enable_grad():
                ...
                loss = F.cross_entropy(logits, torch.tensor([1]), size_average=False)
                # print(loss)
                grad_w = torch.autograd.grad(loss, [self.basic_model.conv1.weight], create_graph=True)[0]
                loss_w = F.mse_loss(grad_w, target)
            grad_x = torch.autograd.grad(loss_w, [x], create_graph=True)[0]
